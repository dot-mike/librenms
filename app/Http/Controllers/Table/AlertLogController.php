...
class AlertLogController extends TableController
{
    ...
    protected function filterFields($request): array
    {
        return [
            ...
            'state' => function ($query, $state) {
                // Skip filtering if state is -1 (all states) or empty
            ...
            'min_severity' => function ($query, $min_severity) {
                if (is_numeric($min_severity) && $min_severity !== '') {
                    ...
                    switch ($min_severity) {
                        ...
                        default:
                            return $query; // No filtering for unrecognized values
                    }
                }
                return $query;
            },
    ...
    public function formatItem($alertlog): array
    {
        ...
        // Get fault details using modern Laravel methods
        if ($alert_state == '0') {
        ...
        // Add verbose details for admins
        if (Auth::user()->hasGlobalAdmin()) {
            $result['verbose_details'] = "<button type='button' class='btn btn-alert-details verbose-alert-details' style='display:none' aria-label='Details' id='alert-details' data-alert_log_id='{$alertlog->id}'><i class='fa-solid fa-circle-info'></i></button>";
        }
    ...
    /**
     * Format alert details from the details array (modernized version of alert_details function)
     */
    ...
    /**
     * Format individual alert detail item (modernized version of format_alert_details function)
     */
    private function formatAlertDetailsItem(int $alert_idx, array $tmp_alerts, ?string $type_info = null): string
    {
        ...
        if (isset($tmp_alerts['port_id'])) {
            if (!empty($tmp_alerts['isisISAdjState'])) {
                ...
                // Use modern Eloquent relationships instead of legacy cleanPort
                $port = \App\Models\Port::find($tmp_alerts['port_id']);
                if ($port) {
        ...
        if (isset($tmp_alerts['accesspoint_id'])) {
            // Handle access point alerts - would need to create generate_ap_link equivalent
        ...
        if (isset($tmp_alerts['sensor_id'])) {
            ...
            // Use modern sensor link generation
